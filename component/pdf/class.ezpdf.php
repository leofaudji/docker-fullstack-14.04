...
function GetPrintDialog($cKey,$cDefault){
  $cValue = $cDefault ;
  if(session_is_registered($cKey)) $cValue = $_SESSION [$cKey] ;
  $_SESSION [$cKey] = "" ;
  if(empty($cValue)) $cValue = $cDefault ;
...
function getText($cValue){
  ...
  foreach($va as $key=>$value){
    $cValue = str_replace($value,'',$cValue) ;
  }
  return $cValue ;
}

function Array2CSV($vaArray,$lShowHeader=true){
  ...
  foreach($vaArray as $key=>$value){
    ...
    $c = '"' . join('","',$this->getText($value)) . '"' . chr(13) . chr(10) ;
    fwrite($nhd,$c) ;
  }
  fclose($nhd) ;
}
...
function Cezpdf($paper='LETTER',$orientation='portrait',$pagenumber='1',$nPaperIncWidth=0,$nPaperIncHeight=0){
  $nTopMargin = 1 ;
  $nLeftMargin = 1.3 ;
  $nBottomMargin = 1 ;
  $nRightMargin = 0.7 ;
  $this->nExportCSV = 0 ;
  if($this->GetPrintDialog("print_Dialog",0) == 1){
    if(empty($nPaperIncWidth)) $nPaperIncWidth = $this->GetPrintDialog("print_Width",0) ;
    if(empty($nPaperIncHeight)) $nPaperIncHeight = $this->GetPrintDialog("print_Height",0) ;

    $nTopMargin = $this->GetPrintDialog("print_Top",10)/10 ;
    $nLeftMargin = $this->GetPrintDialog("print_Left",13)/10 ;
    $nBottomMargin = $this->GetPrintDialog("print_Bottom",10)/10 ;
    $nRightMargin = $this->GetPrintDialog("print_Right",7)/10 ;
    $this->nExportCSV = $this->GetPrintDialog("print_ExportCSV",0) ;
  }
  ...
  if (!is_array($paper)){
    switch (strtoupper($paper)){
      ...
      case 'BUKUTABUNGAN': {$size = array(0,0,405.35,850.40); break;}
  ...
  // Default Setting
  $this->selectFont(compFolder() . '/pdf/fonts/Helvetica.afm');
  $this->ezSetCmMargins($nTopMargin,$nBottomMargin,$nLeftMargin,$nRightMargin) ;
  if ($pagenumber == '1'){
    $this->ezSisPageNumber() ;
  }
   
  //$this->ezHeaderInfo($orientation) ;   
}

function ezHeaderInfo($orientation){
  $nLeft = $this->ez['pageWidth'] - $this->ez['rightMargin'] ;
  $nTop = $this->ez['pageHeight'] - 30 ;
  if(strtolower($orientation) == "portrait"){
    $nLeft = $nLeft-540 ;
  }else{
    $nLeft = $nLeft-780 ;
  } 
  $this->ezStartPageNumbers($nLeft,$nTop+15,8,'right',aCfg("msInfo_Nama")) ;
  $this->ezStartPageNumbers($nLeft,$nTop+7,8,'right',aCfg("msInfo_Alamat")) ;
  $this->ezStartPageNumbers($nLeft,$nTop-2,8,'right','Telp. ' . aCfg("msInfo_Telepon")) ;
} 
...
function ezStream($options=''){
  if(empty($this->nExportCSV)){
    ...
    echo('<a href="' . $this->cFileCSV . '">Click Disini</a> Untuk Downloads File CSV') ;
  }
}
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  $defaults = array('shaded'=>0,'showLines'=>2,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>8,'titleFontSize'=>12
  ...
  if($this->nExportCSV == 1){
    $this->Array2CSV($data,$options ['showHeadings'] == 1) ;
    return true ;
  }
...
/**************************************************************************************************************************************************************************************************/
// Function Buatan Assistindo
/**************************************************************************************************************************************************************************************************/
function ezPageHeader($cText,$vaOption = ''){
  if($this->nExportCSV == 1){
    $va1 [0] = array(""=>$cText) ;
    $this->Array2CSV($va1,false) ;
    return true ;
  }
  ...
  // Hitung Berapa Lebar Tempat Untuk Mencetak 
  ...
  $nCount = 5 ;
  $nTop = ($this->ez['pageHeight'] - $this->ez['topMargin']) - $nTextHeight ;  
  if ($this->nPageHeaderLine !== 0 ){
    $nTop += 5 ;
    $nCount = 0 ;
  }
  $this->nPageHeaderLine += 1 ;  
  $this->ezStartPageNumbers($nLeft,$nTop,$vaOption['fontSize'],'left',$cText) ;
...
/**************************************************************************************************************************************************************************************************/
function ezSisPageNumber(){
  $nLeft = $this->ez['pageWidth'] - $this->ez['rightMargin'] ;
  $nTop = $this->ez['pageHeight'] - 30 ;
  $this->ezStartPageNumbers($nLeft,$nTop,8,'left','Page : {PAGENUM} of {TOTALPAGENUM}',1) ;
  $this->ezStartPageNumbers($nLeft,$nTop-8,8,'left',date("d-m-Y H:i:s ") . GetSetting("cSession_UserName") ) ;
}

function InfoBank(){
  $nLeft = $this->ez['pageWidth'] - $this->ez['rightMargin'] ;
  $nTop = $this->ez['pageHeight'] - 30 ;
  $vaInfo = GetInfoBank(aCfg('msKodeCabang')) ;
  $cNamaBank = aCfg('msNama') . " (" . $vaInfo['NamaBank'] . ")" ;
  $cAlamat = $vaInfo['AlamatBank'] ;
  $cNoTelepon = "No. Telp : " . $vaInfo['TeleponBank'] . ",Fax : " . $vaInfo['FaxBank'] ;
  $this->ezStartPageNumbers($nLeft-630,$nTop+10,8,'left',$cNamaBank) ;
  $this->ezStartPageNumbers($nLeft-786,$nTop,8,'right',$cAlamat) ;
  $this->ezStartPageNumbers($nLeft-786,$nTop-10,8,'right',$cNoTelepon) ;
 } 
}
?>